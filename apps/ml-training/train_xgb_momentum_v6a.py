"""
TICKET-114 — Model V6a. Isolated test: does atrPercentile5m + bbWidthPercentile15m ALONE (dropping
every other feature) beat momentumScore's own AUC (0.507) on the 1,730-candidate momentum-gate slice?
Motivated by a finding that atrPercentile5m (AUC=0.420) and bbWidthPercentile15m (AUC=0.421) each
individually discriminate win/loss BETTER than the model's own combined momentumScore — this checks
whether combining JUST these 2 (no other feature diluting them) does better than either alone.

Same data source, label, and chronological 60/20/20 split + purge convention as
train_xgb_momentum_v5b.py (TICKET-112) — ONLY the feature set changes (2 numeric features, NO
categorical features at all, per the ticket's explicit "bỏ hết feature khác").

EXPERIMENTAL TRAINING ONLY (TICKET-114) — does not touch any production file/model. Output
models/xgb_momentum_v6a_experimental.onnx + _feature_schema.json are NOT wired into anything.

Run: apps/ml-training/.venv/Scripts/python.exe apps/ml-training/train_xgb_momentum_v6a.py
...
MODEL_NAME = "xgb_momentum_v6a_experimental"
ONNX_PATH = os.path.join(MODELS_DIR, f"{MODEL_NAME}.onnx")
SCHEMA_PATH = os.path.join(MODELS_DIR, f"{MODEL_NAME}_feature_schema.json")

# TICKET-114 V6a: ONLY these 2 features — no other numeric feature, NO categorical features at all.
NUMERIC_FEATURES = ["atrPercentile5m", "bbWidthPercentile15m"]
CATEGORICAL_FEATURES: list[str] = []

HORIZON_CANDLES = 10
OVERFIT_AUC_GAP_THRESHOLD = 0.15

# Identical hyperparameters to V5b — only the feature set is the isolated variable.
XGB_PARAMS = dict(
...
def write_feature_schema(feature_cols: list[str]) -> None:
    schema = {
        "numeric_features": NUMERIC_FEATURES,
        "categorical_feature_order": CATEGORICAL_FEATURES,
        "categorical_features": {},
        "missing_categorical_value": None,
...
def main() -> None:
    ...
    print(f"V6a — CHỈ {len(feature_cols)} feature: {feature_cols}")

    os.makedirs(MODELS_DIR, exist_ok=True)
    write_feature_schema(feature_cols)
    ...
    print(f"→ {ONNX_PATH}")
    print("")
    print("TICKET-114: model thực nghiệm (2 feature: atrPercentile5m + bbWidthPercentile15m) — KHÔNG wire vào production.")
