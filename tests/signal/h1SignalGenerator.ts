...
// Tham số theo TICKET-11X-SIGNAL-GENERATOR (H1) — không tối ưu.
const ADX_TREND = 25;
const ADX_SIDEWAY = 20;
...
function main() {
    ...
    console.log(`Đã nạp ${candles.length} nến H1 (${SUFFIX}).`);

    const close = candles.map((c) => c.close);
    const { adx, plusDI, minusDI } = calculateADX(candles, ATR_PERIOD);
    const ema200 = calculateEMA(close, EMA_PERIOD);
    ...
    for (let i = WARMUP; i < candles.length - 3; i++) {
        ...
        if (plusDI[i] > minusDI[i] && close[i] > ema200[i]) direction = 'UP';
        else if (minusDI[i] > plusDI[i] && close[i] < ema200[i]) direction = 'DOWN';
    ...
    console.log(`Trải trên ${months.size} tháng, trung bình ${(stats.accepted / Math.max(1, months.size)).toFixed(1)} lệnh/tháng.`);

    writeFileSync(resolve(RESULTS_DIR, 'h1-signals.json'), JSON.stringify(accepted, null, 2));
    writeFileSync(resolve(DATA_DIR, 'ticket11x-h1-funnel-stats.json'), JSON.stringify(stats, null, 2));
    console.log(`\nĐã lưu ${accepted.length} tín hiệu vào data/results/h1-signals.json`);
    console.log('Đã lưu funnel stats vào data/ticket11x-h1-funnel-stats.json');
}
