"""
TICKET-097 — so sánh công bằng: giữ NGUYÊN nhãn % cũ (label_bullish_momentum/label_bearish_momentum,
data/training/momentum-labeled-v3.csv, generateMomentumTrainingDataV3.ts), CHỈ thêm 2 feature sạch
correlatedRiskRatio + distanceToNearestSwingAtr. KHÔNG sửa train_xgb_momentum.py/v2.py gốc hay
models/xgb_momentum_v1*.onnx — output riêng (xgb_momentum_v3_*_experimental.onnx), CHƯA nối vào
orchestrator/backtest.

Run: apps/ml-training/.venv/Scripts/python.exe apps/ml-training/train_xgb_momentum_v3.py --label-column=label_bullish_momentum --model-name=xgb_momentum_v3_bullish_experimental
"""
...
_parser.add_argument("--model-name", default="xgb_momentum_v3_bullish_experimental")
_args = _parser.parse_args()

CSV_PATH = os.path.join(REPO_ROOT, "data", "training", "momentum-labeled-v3.csv")
...
# feature gốc giữ nguyên thứ tự. lossStreakBullish/lossStreakBearish CÓ trong momentum-labeled-v3.csv
...
# TICKET-097: khớp generateMomentumTrainingDataV3.ts's HORIZON_CANDLES (10, GIỮ NGUYÊN bản % gốc).
R_LABEL_HORIZON_CANDLES = 10
