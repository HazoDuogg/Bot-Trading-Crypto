"""
TICKET-110 — Model V4. Mirrors train_xgb_momentum.py's exact conventions (chronological sort by
...
data/training/momentum-v4-labeled.csv's `label_win` column (REAL trade outcome, TICKET-109) instead of
the Fixed-Time-Horizon `label_bullish_momentum` column — the ONE isolated variable this ticket tests.
Same 10 features as V1 (adx1h, atrPercentile5m, bbWidthPercentile15m, volumeZScore5m, atrTrend5m,
adxDirection1h, macroDirection, volAdjReturn5m, emaRatioFast, emaRatioSlow), `symbol` included as an
extra categorical feature too — same precedent V1's own script already set (not a new feature).

Only intentional deviation from V1's script: no `--label-column`/`--model-name` CLI args (this ticket
trains exactly one fixed target), and CSV_PATH/output names point at the V4 artifacts instead.

EXPERIMENTAL TRAINING ONLY (TICKET-110) — does not touch train_xgb.py, train_xgb_momentum.py, or any
production model/config referenced by apps/bot/src/xgbFilter/config.ts. Output
models/xgb_momentum_v4_experimental.onnx + _feature_schema.json are NOT wired into anything.

Run: apps/ml-training/.venv/Scripts/python.exe apps/ml-training/train_xgb_momentum_v4.py
(after `npx tsx apps/bot/scripts/ticket110GenerateV4TrainingData.ts` from repo root)
"""
...
# Must match ticket110GenerateV4TrainingData.ts's per-(symbol,timestamp) granularity — same
# HORIZON_CANDLES=10 purge convention V1's script uses (see its own comment for the caveat: a
# row-count purge is smaller in real time/step terms than a literal per-symbol candle gap, here even
# more so since each timestamp appears twice — one row per side. Same known limitation, not silently
# ignored).
HORIZON_CANDLES = 10

OVERFIT_AUC_GAP_THRESHOLD = 0.15

# Identical to V1's XGB_PARAMS — the only variable this ticket isolates is the label, not the
# hyperparameters.
XGB_PARAMS = dict(
...
def main() -> None:
    ...
    # Chronological sort, NO shuffling — same convention as V1's script.
    ...
    print(f"→ {ONNX_PATH}")
    print("")
    print("TICKET-110: đây là model thực nghiệm (label_win) — KHÔNG wire vào production/xgbFilter/config.ts.")
