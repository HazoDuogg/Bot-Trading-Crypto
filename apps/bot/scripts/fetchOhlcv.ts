...
async function main() {
  const [symbol = 'BTCUSDT', interval = '1h', monthsBackArg = '6'] = process.argv.slice(2);
  const monthsBack = Number(monthsBackArg);
  ...
  const outPath = path.join(dataDir, `${symbol}_${interval}.csv`);
