/**
 * TICKET-125 Việc A/B/C — extended shadow-audit script. Same STANDALONE, READ-ONLY walk/windowing
 * convention as TICKET-124's `ticket124TacticalShadowAudit.ts` (never touches src/ production
 * defaults, never opens a real position) — this is a SEPARATE script/output, TICKET-124's own
 * script and CSV are left completely untouched (immutable baseline for comparison).
 *
 * Adds, on top of TICKET-124's row shape:
 *   - Việc A raw-feature columns not already in the TICKET-124 CSV: volAdjReturn5m, bodyRatio,
 *     upperWickRatio, lowerWickRatio, directionFlips, choppyScore — all computed via
 *     tactical/tacticalRegimeClassifier.ts's own EXPORTED helpers (candleDirection, bodyRatioOf,
 *     countDirectionFlips) and TacticalConfig thresholds, never a re-derived formula.
 *   - Việc B structural-break fix: `hasStructuralBreakOld` reproduces TICKET-124's exact
 *     direction-agnostic/unstaled proxy (adxDirection1h macro proxy, whole-200-1m-candle window,
 *     no staleness filter) side-by-side with the NEW sided+recent+swing-anchored
 *     `detectRecentStructuralBreak()` (entry/detectors/structuralBreakRecent.ts) computed for BOTH
 *     BULLISH and BEARISH, fed into the tactical classifier as the `{ LONG, SHORT }` form so
 *     STRUCTURAL_BREAK is checked against the SAME side the candidate itself is being scored for.
 *     Richer columns logged: structuralBreakSide, structuralBreakTimestamp,
 *     structuralBreakAgeCandles, structuralBreakLevel, distanceFromBreakAtr (reusing the same
 *     wilderATRSeries(window5m, RegimeConfig.ATR_PERIOD_5M) ATR already computed for the shadow
 *     simulator, not a new ATR formula) — all for whichever side the CONFIRMED tactical output
 *     picked (side=null states log these as NA, since there's no side to anchor to).
 *
 * Run (from repo root, after `npm run build`): `npm run build:scripts && node apps/bot/scripts-dist/ticket125StructuralBreakAudit.js`
 */
...
if (process.env.TACTICAL_REGIME_5M_ENABLED === 'true') {
  throw new Error('ticket125StructuralBreakAudit: TACTICAL_REGIME_5M_ENABLED must stay false — this script is the shadow-only audit path, never a production gate.');
}
const SHADOW_ON = process.env.TACTICAL_REGIME_5M_SHADOW !== 'false';

const SYMBOLS = ['BTCUSDT', 'ETHUSDT', 'SOLUSDT', 'XRPUSDT'];
const OHLCV_DIR = path.resolve(process.cwd(), 'data/ohlcv');
const OUT_CSV_PATH = path.resolve(process.cwd(), 'data/ticket125-tactical-shadow-audit-v2.csv');
...
// ---- Official 8-flag baseline (same as TICKET-124/TICKET-125 spec) — dataset/period/warmup match exactly. ----
const SKIP_DAYS = 20;
...
/** Identical simplified simulator to TICKET-124's — same formula/limitations, documented there and in the TICKET-125 report. */
...
async function main(): Promise<void> {
  console.log(`TICKET-125 Việc A/B/C — extended shadow audit. TACTICAL_REGIME_5M_SHADOW active=${SHADOW_ON}`);
  ...
  const header = [
    ...
    // Việc A raw features not already in the TICKET-124 CSV
    'volAdjReturn5m', 'bodyRatio', 'upperWickRatio', 'lowerWickRatio', 'directionFlips', 'choppyScore', 'atrTrend5m', 'candleDir',
    // Old (TICKET-124) vs new (TICKET-125) structural break booleans
  ...
  for (let step = warmupStartStep; step < totalSteps; step++) {
    ...
    for (const symbol of SYMBOLS) {
      ...
      // ---- TICKET-125 Việc B: NEW sided, recent, swing-anchored structural break — BOTH directions. ----
      ...
      // ---- Tactical Shadow classifier — per-side structural break form ----
...
main().catch((err) => {
  console.error('ticket125StructuralBreakAudit failed:', err);
  process.exit(1);
});
