...
// TICKET-RT-076: same 14-feature extraction as rt065FeatureAuditThreeYear.ts (RT-065 Part D),
// not modified there — this is a parameterized copy so it can also self-check against the
// ORIGINAL (XRP) lineup on 1y data before trusting the DOGE-lineup 3y run.
...
const ORIGINAL_LEVERAGE: Record<string, number> = { BTCUSDT: 20, ETHUSDT: 20, SOLUSDT: 10, HYPEUSDT: 10, XRPUSDT: 10 };
...
const DOGE_LEVERAGE: Record<string, number> = { BTCUSDT: 20, ETHUSDT: 20, SOLUSDT: 10, HYPEUSDT: 10, DOGEUSDT: 10 };
...
function checkGridsAlignExactly(allData: Map<string, SymbolData>, referenceSymbols: string[]): GridCheckResult {
  ...
  for (const sym of referenceSymbols.slice(1)) {
    ...
    if (d.m15All.length !== ref.m15All.length || d.h1All.length !== ref.h1All.length) {
      throw new Error(`CORRECTION_REQUIRED: ${sym} co do dai grid khac ${referenceSymbols[0]} — dung lai.`);
    }
...
function runInstrumentedSimulation(
...
): { rows: OutputRow[]; closed: ClosedTradeInternal[] } {
  ...
  for (let i = 2; i < grid.referenceM15Length; i++) {
    ...
    for (const st of states) {
      ...
      const ntz = checkNoTradeZone({ nowMs: m15CloseTime, bid: closePrice, ask: closePrice, h1Candles: h1Window, m15Candles: m15Window });
      ...
      if (st.pending) {
        ...
        if (touchedGap && !ntz.blocked) {
          ...
          if (slDistance > 0) {
            ...
            const sizing = calculatePositionSize({ balance: BALANCE, riskUsd, entryPrice, slPrice, leverage: leverage[symbol], maxMarginPct: DEFAULT_MAX_MARGIN_PCT });
...
function toCsv(rows: OutputRow[]): string {
  ...
  const lines = rows.map((r) => CSV_COLUMNS.map((c) => cell((r as unknown as Record<string, unknown>)[c])).join(','));
...
async function main() {
  ...
  console.log('=== Self-check lop 1: bo coin GOC (XRP), du lieu 1 nam, doi chieu RT-056/057 ===');
  ...
  const { closed: closed1y } = runInstrumentedSimulation(data1y, grid1y, targetR, ORIGINAL_SYMBOLS, ORIGINAL_REFERENCE_SYMBOLS, ORIGINAL_LEVERAGE);
  ...
  if (!(s1y.n === 1217 && Math.abs(s1y.pnl - 2628.76) < 0.01 && Math.abs(s1y.profitFactor - 1.551) < 0.001)) {
    console.error('CORRECTION_REQUIRED: khong khop RT-056/057 — DUNG lai.');
    process.exitCode = 1;
    return;
  }
  console.log('-> KHOP 100%.\n');

  console.log('=== Self-check lop 2: lineup DOGE, du lieu 3 nam, doi chieu RT-DOGE-001 ===');
  ...
  const { rows, closed } = runInstrumentedSimulation(dataDoge, gridDoge, targetR, DOGE_SYMBOLS, DOGE_REFERENCE_SYMBOLS, DOGE_LEVERAGE);
  console.log(`Chay xong trong ${((Date.now() - startTime) / 1000).toFixed(1)}s.`);
  const sDoge = summarizeForCheck(closed);
  console.log(`n=${sDoge.n}  PnL=$${sDoge.pnl.toFixed(2)}  PF=${sDoge.profitFactor.toFixed(3)} (doi chieu RT-DOGE-001: n=3804, PnL=$7642.57, PF=1.451)`);
  if (!(sDoge.n === 3804 && Math.abs(sDoge.pnl - 7642.57) < 0.01 && Math.abs(sDoge.profitFactor - 1.451) < 0.001)) {
    console.error('CORRECTION_REQUIRED: khong khop RT-DOGE-001 — feature extraction co the da lam thay doi quyet dinh admission — DUNG lai, khong ghi dataset.');
    process.exitCode = 1;
    return;
  }
  console.log('-> KHOP 100%: feature extraction khong lam thay doi tap lenh cua RT-DOGE-001.\n');

  const outPath = path.resolve(process.cwd(), 'apps/bot/data/xgbAuditDatasetDoge.csv');
